...
- 修复大阶乘计算溢出问题
- 输入文字字体调整大小，适应不同屏幕尺寸
- 修复科学计算器中，输入的表达式中包含'i'时，显示无效输入的问题
- 使用千分位分隔符显示方式
- 删除大数字的手动处理逻辑，切换到使用math.js的自动格式化功能
- 修复了updateCurrentText状态未正确更新的问题
- 修改mathjs配置以使用BigNumber并将精度设置为64位
...
- 改进键盘按钮的自适应布局
- 优化大屏幕显示效果
- 组件新增属性：`angle-unit`（角度单位）, `theme`（主题）, `language`（语言）, `initial-mode`（初始计算器模式）
- 新增事件触发：`mode-change`（模式改变时触发）, `language-change`（语言改变时触发）, `theme-change`（主题改变时触发）, `calculation`（执行计算时触发）
